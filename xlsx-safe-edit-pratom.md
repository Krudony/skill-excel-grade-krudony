...
- **เกรด 1, 1.5, 2 → BH ≤ 10 และ DI ≤ 10 เสมอ** (ห้ามให้คะแนนสอบสูงแต่เกรดต่ำ ดูขัดแย้ง)
  - ถ้า BH เดิม > 10 → ลด BH ให้ ≤ 10 แล้วเพิ่ม BJ-BQ ชดเชย (BI เปลี่ยน ต้องอัพเดท cached BI ด้วย)
...
### 🪞 Mirror Pattern — ใช้ภาค1 เป็น template ภาค2

**หลักการ:** นักเรียนคนเดียวกัน ตัวชี้วัดที่เก่งในภาค1 ควรยังเก่งกว่าตัวอื่นในภาค2 ด้วย รูปแบบควรสอดคล้องกัน ต่างกันแค่ระดับ

**วิธีคำนวณ:**
```
1. คำนวณ DH_target = DQ_aim - BI - DI
2. scale_factor = DH_target / sum(ภาค1_indicators)
3. ภาค2[i] = round(ภาค1[i] * scale_factor) → ปรับให้ sum ตรง
4. ตรวจ balance: sum(ภาค2[:4]) ≈ sum(ภาค2[4:]) (ห้ามต่างเกิน 1-2)
5. ตรวจ max diff ต่อตำแหน่ง: ควร ≤ 2 (หรือ uniform ถ้า > 2)
```

**ตัวอย่างจริง (ปพ.5 ป.2 กอท 2568 ภาค2):**

| ชื่อ | เกรด | BI | ภาค1 pattern | ภาค2 pattern | DH | DI | max diff |
|------|------|----|-----------|-----------|----|-----|---------|
| กรวิชญ์ | 3 | 80 | [6,7,**8**,7,7,7,7,7] | [5,6,**6**,5,5,6,5,6] | 44 | 20 | -2 |
| ชินภัทธ์ | 3 | 77 | [7,**8**,7,6,7,7,7,7] | [6,**7**,6,5,6,6,6,6] | 48 | 19 | -1 (uniform) |
| ธนกฤต | 2 | 69 | [7,**8**,7,**8**,7,7,**8**,**8**] | [6,**7**,6,**7**,6,6,**7**,**7**] | 52 | 3 | -1 (uniform) |
| ชนัดดา | 2.5 | 84 | [**8**,9,9,9,**8**,**8**,9,9] | [**5**,6,6,6,**5**,**5**,6,6] | 45 | 5 | -3 (uniform) |
| พิชญาภัค | 3.5 | 81 | [7,6,7,7,6,6,7,7] | [7,6,7,7,6,6,7,7] | 53 | 20 | 0 (เหมือนกัน) |

**กรณีพิเศษ — ภาค1 = all minimum (all 5s):**
- ไม่สามารถ mirror ได้เพราะ 5 คือขีดต่ำสุด
- ให้ใช้ DH ที่น้อยที่สุดเท่าที่ได้เกรดเป้าหมาย พร้อม DI ≤ 10
- ยอมรับ ภาค2 สูงกว่าภาค1 นิดหน่อย (จำเป็นทางคณิตศาสตร์)
- ตัวอย่าง: BI=48, grade 1 → DH=44, DI=8, DQ=100, DR=50 ✓

**🔴 กฎ Balance — ต้องตรวจทุกคนก่อน save:**

> คะแนนเก็บภาค1 และภาค2 ต้องกระจายสม่ำเสมอ **ไม่หนักไปข้างใดข้างหนึ่ง** ทั้งภายในแต่ละภาค และเปรียบเทียบระหว่างภาค

**ตรวจ 3 ระดับ:**

```
① Balance ภายในภาค (first 4 vs last 4):
   sum(ind[:4]) - sum(ind[4:]) ≤ 1  → ✅ OK
   ≤ 2                               → ⚠️ ยอมรับได้
   > 2                               → 🔴 ต้องปรับ (สลับค่าระหว่างครึ่งแรก-หลัง)

② Consistency ภาค1 → ภาค2 (per position):
   max |ภาค2[i] - ภาค1[i]| ≤ 2     → ✅ OK
   = 3 แต่ uniform ทุกตำแหน่ง       → ⚠️ ยอมรับได้
   > 3 หรือไม่ uniform               → 🔴 ต้องปรับ (ใช้ mirror pattern)

③ ทิศทางสอดคล้องกับเกรด:
   เกรดขึ้น → ภาค2 ≥ ภาค1          → ✅ สมเหตุ
   เกรดลง  → ภาค2 ≤ ภาค1          → ✅ สมเหตุ
   เกรดลงแต่ indicators ขึ้น        → ⚠️ ต้องมีเหตุผล (เช่น BH/DI ต่ำมาก)
```

**ลำดับตรวจสอบก่อน save:**
1. ✅ ทุก BJ-BQ ≥ 5
2. ✅ balance ภาค1: sum(I-L) ≈ sum(M-P) ต่างกันไม่เกิน 2
3. ✅ balance ภาค2: sum(BJ-BM) ≈ sum(BN-BQ) ต่างกันไม่เกิน 2
4. ✅ pattern ภาค2 สะท้อน pattern ภาค1 (mirror)
5. ✅ เกรด 1, 1.5, 2 → BH ≤ 10, DI ≤ 10
6. ✅ DQ = BI + DH + DI → DR อยู่ใน grade range
